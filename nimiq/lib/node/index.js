...
// lib/Entropy.ts
var import_core4 = require("../../nodejs/index.js");

// lib/ExtendedPrivateKey.ts
var import_core = require("../../nodejs/index.js");
...
var ExtendedPrivateKey = class _ExtendedPrivateKey extends Serializable {
  ...
  constructor(key, chainCode) {
    super();
    if (!(key instanceof import_core.PrivateKey)) throw new Error("ExtendedPrivateKey: Invalid key");
  ...
  static generateMasterKey(seed) {
    ...
    const hash = import_core.CryptoUtils.computeHmacSha512(bCurve, seed);
    return new _ExtendedPrivateKey(new import_core.PrivateKey(hash.slice(0, 32)), hash.slice(32));
  }
  /**
  ...
  derive(index) {
    ...
    const data = new SerialBuffer(1 + import_core.PrivateKey.SIZE + 4);
    data.writeUint8(0);
    data.write(this._key.serialize());
    data.writeUint32(index);
    const hash = import_core.CryptoUtils.computeHmacSha512(this._chainCode, data);
    return new _ExtendedPrivateKey(new import_core.PrivateKey(hash.slice(0, 32)), hash.slice(32));
  }
  /**
  ...
  static deserialize(buf) {
    const privateKey = import_core.PrivateKey.deserialize(buf);
   ...
   */
  toAddress() {
    return import_core.PublicKey.derive(this._key).toAddress();
  }
};

// lib/MnemonicUtils.ts
var import_core2 = require("../../nodejs/index.js");
...
var MnemonicUtils = class _MnemonicUtils {
  ...
  static mnemonicToSeed(mnemonic, password) {
    ...
    return new SerialBuffer(import_core2.CryptoUtils.computePBKDF2sha512(mnemonicBuffer, saltBuffer, 2048, 64));
  }
  /**
  ...
  static _sha256Checksum(entropy) {
    const ENT = entropy.length * 8;
    const CS = ENT / 32;
    const hash = import_core2.Hash.computeSha256(entropy);
    return _MnemonicUtils._toBinary(hash).slice(0, CS);
  }
...
// lib/Secret.ts
var import_core3 = require("../../nodejs/index.js");
var Secret = class _Secret extends Serializable {
  ...
  static async exportEncrypted(secret, key) {
    const salt = import_core3.CryptoUtils.getRandomValues(_Secret.ENCRYPTION_SALT_SIZE);
    const data = new SerialBuffer(
      /*purposeId*/
      4 + _Secret.SIZE
    );
    if (secret instanceof import_core3.PrivateKey) {
      data.writeUint32(import_core3.PrivateKey.PURPOSE_ID);
    ...
    const checksum = import_core3.Hash.computeBlake2b(data).subarray(0, _Secret.ENCRYPTION_CHECKSUM_SIZE_V3);
    const plaintext = new SerialBuffer(checksum.byteLength + data.byteLength);
    plaintext.write(checksum);
    plaintext.write(data);
    const ciphertext = await import_core3.CryptoUtils.otpKdf(plaintext, key, salt, _Secret.ENCRYPTION_KDF_ROUNDS);
  ...
  static async _decryptV3(buf, key, rounds) {
    ...
    4 + _Secret.SIZE);
    const plaintext = await import_core3.CryptoUtils.otpKdf(ciphertext, key, salt, rounds);
    ...
    const checksum = import_core3.Hash.computeBlake2b(payload).subarray(0, _Secret.ENCRYPTION_CHECKSUM_SIZE_V3);
    if (!BufferUtils.equals(check, checksum)) {
      throw new Error("Invalid key");
    }
    ...
    switch (purposeId) {
      case import_core3.PrivateKey.PURPOSE_ID:
        return new import_core3.PrivateKey(secret);
...
var Entropy = class _Entropy extends Secret {
   ...
   */
  static generate() {
    const entropy = import_core4.CryptoUtils.getRandomValues(_Entropy.SIZE);
